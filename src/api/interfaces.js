...
const interfaces = {
  ...
  async getSession(code) {
    const url = `${wxUrl}/weixin/miniApp2/getMiniAppSessionIdPlus`;
   ...
   * 解密用户手机号
   */
  async decodePhoneNumber(miniAppSessionId, encryptedData, iv,companyName) {
    const url = `${wxUrl}/weixin/miniApp2/decodePhoneNumber`;
    const data = await http.post(url, {miniAppType: this.miniAppType, miniAppSessionId, encryptedData, iv,companyName:companyName},{}, {
      isLoading: true
    });
    return data;
  },
   ...
   * 获取用户手机号
   */
  async getPhoneNumber(e,companyName) {
    const sessionId = wx.getStorageSync('sessionId');
    let phoneData = await this.decodePhoneNumber(sessionId, e.mp.detail.encryptedData, e.mp.detail.iv,companyName);
